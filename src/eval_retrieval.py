...
import sys
sys.path.append('/kaggle/working/pic2word')
import os
import time
import logging
...
from eval_utils import evaluate_imgnet_retrieval, evaluate_coco, evaluate_fashion, evaluate_cirr, evaluate_cirr_test, evaluate_circo
...
def main_worker(gpu, ngpus_per_node, log_queue, args):
    ...
    if args.eval_mode == 'coco':
        ...
        source_dataset = CIRCODataset(dataset_path=root_project, split='val', mode='relative', preprocess=preprocess_val, eval='source')
        target_dataset = CIRCODataset(dataset_path=root_project, split='val', mode='relative', preprocess=preprocess_val, eval='target') 
        source_dataloader = DataLoader(
            source_dataset,
            batch_size=args.batch_size,
            shuffle=False,
