...
## CIRCO: under ./data/CIRCO
## validation images ./COCO2017_unlabeled/unlabeled2017
## validation split of images and captions ./annotations/val.json
## test split of images and captions ./annotations/test.json    
class CIRCODataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[str, Path], split: Literal['val', 'test'],
                 mode: Literal['relative', 'classic'], preprocess: callable, eval: Literal['source', 'target']):
        """
        Args:
        ...
        if split not in ['val', 'test']:
            raise ValueError("split should be in ['val', 'test']")
        ...
        # Get maximum number of ground truth images (for padding when laoding the images)
    ...
    def __getitem__(self, index) -> Dict:
        ...
        In 'relative' mode, the dataset yields dictionaries with the following kyes:
            - [reference_ig, reference_img_id, target_img, target_img_id, relative_caption, shared_concept, gt_img_ids,
        ...
        if self.mode == 'relative':
            ...
            if self.split == 'val':
                ...
                assert self.eval in ['source', 'target']
                if self.eval == 'source':
                    return {
