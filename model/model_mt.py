"""
Multi-token fusion version of pic2word module
"""
...
class IM2TEXT(nn.Module):
    def __init__(self, embed_dim=512, middle_dim=512, output_dim=512, n_layer=2, dropout=0.1, token_num=5):
        super().__init__()
        layers = []
        dim = embed_dim
        ...
        self.ffn1 = nn.Linear(middle_dim, output_dim*token_num)

        # Attach weights layer to compose tokens
        self.ffn2 = nn.Sequential(
            nn.Dropout(dropout), 
            nn.ReLU(),
            nn.Linear(output_dim*token_num, output_dim)
        )
        # self.trained_weights = nn.Parameter(torch.randn(token_num), requires_grad=True)
